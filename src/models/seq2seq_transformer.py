...
class Seq2SeqTransformer(torch.nn.Module):
    def __init__(self, model_config, word2index):
        ...
        self.positional_encoding = PositionalEncoding(
            self.embedding_size, self.maxlen, device=self.device
        )
    ...
    def generate_tgt_msk(self, tgt):
        mask = torch.triu(
            torch.ones(tgt.shape[1], tgt.shape[1],
                       device=self.device) * float('-inf'), diagonal=1
        )
        return mask

    def generate_src_msk(self, src):
        mask = torch.zeros(
            (src.shape[1], src.shape[1]), device=self.device).type(torch.bool)
        return mask
    ...
    def generate_all_masks(self, src, tgt=None):
        ...
        if tgt is not None:
            ...
            tgt_key_padding_mask = (
                tgt == self.word2index["[PAD]"]).type(torch.bool)
        else:
            tgt_mask = None
            tgt_key_padding_mask = None

        src_key_padding_mask = (
            src == self.word2index["[PAD]"]).type(torch.bool)
    ...
    def training_step(self, batch):
        ...
        src_mask, tgt_mask, src_key_padding_mask, tgt_key_padding_mask = (
            self.generate_all_masks(input_tensor, target_tensor)
        )
        ...
        for di in range(target_length):
            loss += self.criterion(
                decoder_outputs[:, di, :].squeeze(),
                target_tensor[:, di, :].squeeze()
            )
    ...
    def validation_step(self, batch):
        ...
        with torch.no_grad():
            ...
            tgt_key_padding_mask) = self.generate_all_masks(input_tensor, target_tensor)
            decoder_outputs = self.forward(
                input_tensor, target_tensor,
                src_mask=src_mask, tgt_mask=tgt_mask,
                src_key_padding_mask=src_key_padding_mask,
    ...
    def encode(self, src, src_mask, src_key_padding_mask=None):
        ...
        src_embed = self.positional_encoding(src_embed)
        src_embed = src_embed.permute([1, 0, 2])
        encoded = self.transformer_model.encoder(
            src_embed, src_mask, src_key_padding_mask)
        src_embed = src_embed.permute([1, 0, 2])

        return encoded


    def decode(self, tgt, encoded, tgt_mask=None, src_mask=None,
        ...
        decoded = self.transformer_model.decoder(
            tgt_embed, encoded,
